...
export default function AdminPage() {
  ...
  const [tableItems, setTableItems] = useState<LessonResponseModel[] | null>(
    null
  );
  ...
  const handleTeacherSelectChange = (event: any) => {
  ...
  const updateLessonModalOpen = (id: string) => {
    LessonService.getApiLessons1(id).then((e) => {
      setLessonName(e.name!);
      setSelectedTeacherId(e.teacherId);
    });
  };
  ...
  const assignStudent = (id: string) => {
    window.location.href = `/assign-lesson-student/${id}`;
  };
  const deleteLesson = (id: string) => {
    LessonService.deleteApiLessons(id).then((e) => {
      LessonService.getApiLessonsByUser().then((e) => {
        setTableItems(e);
      });
    });
  };
  const saveUpdateModal = (id: string) => {
    LessonService.putApiLessons(id, {
      name: lessonName,
      teacherId: selectedTeacherId,
    }).then((e) => {
      LessonService.getApiLessonsByUser().then((e) => {
  ...
  useEffect(() => {
    ...
    if (token) {
      ...
      // dispatch(set(user!.role));
      if (user!.role == "Admin") {
        UsersService.getApiUsersByRole("Teacher").then((e) => {
          setTeachers(e);
        });
      }
          ...
          <table className="w-full table-auto text-sm text-left">
            <thead className="bg-gray-50 text-gray-600 font-medium border-b">
              <tr>
                ...
                <th className="py-3 px-6">Teacher User Name</th>
                ...
                <tr key={idx}>
                  ...
                  <td className="text-right px-6 whitespace-nowrap">
                    ...
                    {userRole == "Admin" ? (
                      <Dialog.Root>
                        <Dialog.Trigger
                          className="py-2 leading-none px-3 font-medium text-yellow-600 hover:text-yellow-500 duration-150 hover:bg-gray-50 rounded-lg"
                          onClick={() => updateLessonModalOpen(item.id!)}
                        >
                          Edit
                        </Dialog.Trigger>
                        <Dialog.Portal>
                          ...
                          <Dialog.Content className="fixed top-[50%] left-[50%] translate-x-[-50%] translate-y-[-50%] w-full max-w-lg mx-auto px-4">
                            <div className="bg-white rounded-md shadow-lg">
                              ...
                              <div className="flex items-center gap-3 p-4 border-t">
                                ...
                                <Dialog.Close asChild>
                                  <button
                                    onClick={() => saveUpdateModal(item.id!)}
                                    className="px-6 py-2 text-white bg-indigo-600 rounded-md outline-none ring-offset-2 ring-indigo-600 focus:ring-2 "
                                  >
                                    Save
                                  </button>
